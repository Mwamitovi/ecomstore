...
class URLCanonicalizationMiddleware(object):
    """ requires the full hostname to which the middleware will redirect to be in settings.py as:
        ...
        One drawback of using this middleware in order to handle canonicalization instead of doing it
        with the web server is the issues that arise with SSL Certificates.
        Imagine that you have a valid cert for 'www.your-domain.com'.
        While attempting to access 'your-domain.com', customers may get a warning message
        about an invalid SSL Cert, and then they'll be redirected to 'www.your-domain.com'.
